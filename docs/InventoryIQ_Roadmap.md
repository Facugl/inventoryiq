...
**Estado: COMPLETADA y ampliada.** 19 casos de uso expuestos vía REST (ver `docs/InventoryIQ_Arquitectura.md` Tabla 1.3), varios sin endpoint documentado en la Sección 8 original (búsqueda de productos, sucursales, categorías, conteo manual de inventario, clasificación ABC/XYZ, forecast, reorder suggestions, y proveedores). `PATCH /api/v1/categories/{id}/parameters` (Sección 8.10) sí estaba documentado desde el diseño original y ahora también está implementado. Documentación OpenAPI vía springdoc-openapi: Swagger UI interactiva en `/swagger-ui/index.html` y el JSON crudo en `/v3/api-docs`, con cada controller anotado (`@Tag`/`@Operation`) agrupado por área funcional. Los 15 mappers de `adapters/in/rest/mapper` (traducen los `*Result` de cada caso de uso a su `*Response` REST) pasaron de clases estáticas escritas a mano a interfaces generadas con MapStruct — una tecnología nueva para el proyecto, no mencionada en `InventoryIQ_Documentacion.md` (cuya tabla de tecnología, Sección 2.6, no baja a ese nivel de detalle de librerías).
...
**Stack de frontend (actualizado):** las 9 pantallas originalmente se implementaron con CSS plano por pantalla y `fetch`/`useEffect`/`useState` a mano para consumir la API. Se migraron a: Material UI + emotion para todos los componentes visuales (reemplazando el CSS plano, con un tema propio que preserva los mismos colores — acento violeta, claro/oscuro según `prefers-color-scheme` — en vez de adoptar la identidad visual por defecto de Material Design); una instancia centralizada de axios con interceptor de respuesta (`api/http.ts`) en vez de `fetch` suelto; TanStack Query para el fetching/cacheo de datos del servidor, reemplazando `useEffect`/`useState` y el flag manual de "cancelled" en cada pantalla; react-hook-form + yup para los tres formularios con validación real (conteo de stock, parámetros de categoría, lead time de proveedor); y react-toastify para el feedback de cada mutación (antes solo un mensaje de error en línea). Redux Toolkit se evaluó y se descartó a propósito: TanStack Query ya cubre el caching de estado de servidor para el que se lo hubiera usado, sin el boilerplate de slice/thunk por entidad.

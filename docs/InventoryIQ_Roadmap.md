...
**Estado: COMPLETADA y ampliada.** 19 casos de uso expuestos vía REST (ver `docs/InventoryIQ_Arquitectura.md` Tabla 1.3), varios sin endpoint documentado en la Sección 8 original (búsqueda de productos, sucursales, categorías, conteo manual de inventario, clasificación ABC/XYZ, forecast, reorder suggestions, y proveedores). `PATCH /api/v1/categories/{id}/parameters` (Sección 8.10) sí estaba documentado desde el diseño original y ahora también está implementado. Documentación OpenAPI vía springdoc-openapi: Swagger UI interactiva en `/swagger-ui/index.html` y el JSON crudo en `/v3/api-docs`, con cada controller anotado (`@Tag`/`@Operation`) agrupado por área funcional. Los 15 mappers de `adapters/in/rest/mapper` (traducen los `*Result` de cada caso de uso a su `*Response` REST) pasaron de clases estáticas escritas a mano a interfaces generadas con MapStruct — tecnología que ya figuraba como prevista en la Sección 6 de `InventoryIQ_Documentacion.md` pero que hasta ahora no se había adoptado.
